...
# Modification by sd2k9 for the tweetfile project - see "sd2k9" tag
#
...
# sd2k9: Import data from the tweetfileaccess.py package
# Fetch uid from command line
if len(sys.argv) <= 1:
  print "Please provide userid on command line"
  sys.exit(0)
uid = int(sys.argv[1])
import tweetfileaccess
try:
  consumer_key    = tweetfileaccess.UserList[uid]['twitter']['consumer_key']
  consumer_secret = tweetfileaccess.UserList[uid]['twitter']['consumer_secret']
except KeyError:
  print "Failed to extract consumer information from tweetfileaccess for userid " + str(uid)
  sys.exit(1)
except:
  raise
